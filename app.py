...
st.markdown("請選擇你要轉換的報告類型，並上載相關的 PDF 檔案。本工具會自動將數據轉換為 Excel 格式，方便稍後貼上至 QSIP 的分析檔案中。")
...
with tab1:
    ...
    # 調整比例：左側(圖片)佔2份，右側(操作)佔5份，這樣圖片就會縮小一半
    col1, col2 = st.columns([2, 5])
    
    with col1:
        st.info("💡 **本區適用於以下格式的報告：**\n\n一般為數學科等科目，表格橫向列出「平均分 Mean」、「標準差 S.D.」等數據。")
        if os.path.exists("example1_item.png"):
            st.image("example1_item.png", caption="項目分析報告格式參考", use_column_width=True)
        else:
            st.warning("⚠️ (提示: 系統未找到 example1_item.png)")
            
    with col2:
        file_item = st.file_uploader("📂 請於此處上載「項目分析」PDF", type=["pdf"], key="file_item")
...
with tab2:
    ...
    # 調整比例：左側(圖片)佔2份，右側(操作)佔5份
    col3, col4 = st.columns([2, 5])
    
    with col3:
        st.info("💡 **本區適用於以下格式的報告：**\n\n一般為英文科的 MCQ 報告，表格列出「A, B, C, D」選項的選擇人數，並附有 ☑️ 標記顯示正確答案。")
        if os.path.exists("example2_mcq.png"):
            st.image("example2_mcq.png", caption="MCQ 報告格式參考", use_column_width=True)
        else:
            st.warning("⚠️ (提示: 系統未找到 example2_mcq.png)")
            
    with col4:
        file_mcq = st.file_uploader("📂 請於此處上載「MCQ 分析」PDF", type=["pdf"], key="file_mcq")
...
# ==========================================
st.divider()
st.caption("📌 小貼士：下載 Excel 後，請打開檔案並全選數據，然後直接貼上 (Ctrl+C 及 Ctrl+V) 至 QSIP 提供的分析範本內。")
