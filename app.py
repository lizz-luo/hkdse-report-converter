...
st.write("請上傳考評局的 PDF 報告（支援**所有科目**，包括中英數），系統會自動按「卷別」精準提取數據。")

uploaded_file = st.file_uploader("請點擊或拖曳上傳 PDF 檔案", type="pdf")

# 專門對付考評局無格線表格的進階參數
custom_table_settings = {
    "vertical_strategy": "text",      # 依靠文字邊界來切分欄位
    "horizontal_strategy": "text",    # 依靠文字邊界來切分列
    "intersection_x_tolerance": 2,    # [關鍵] 縮小X軸容錯率，防止相近的欄位被合併
    "intersection_y_tolerance": 2,    # 縮小Y軸容錯率，確保行與行獨立
    "min_words_vertical": 2           # 垂直方向至少要對齊2個字元才算是一條分隔線
}
    ...
    with st.spinner("正在智能解析 PDF 並精準對齊數據，這可能需要幾秒鐘..."):
        ...
        with pdfplumber.open(uploaded_file) as pdf:
            for page in pdf.pages:
                ...
                # 智能捕捉卷別，並支援中英文標籤 (例如: "卷 Paper: 1", "Paper: 2")
        ...
        if has_data:
            st.success("✅ 智能轉換成功！系統已優化中文科欄位對齊。請點擊下方按鈕下載 Excel。")
            st.download_button(
                label="📥 下載高精度 Excel 檔案",
