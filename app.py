...
# ==========================================
st.set_page_config(page_title="DSE Report Converter | 報告轉換工具", page_icon="📊", layout="wide")

st.title("📊 DSE 考評局報告 PDF 轉 Excel 工具 \n DSE HKEAA Report PDF to Excel Converter")
st.markdown("""
請選擇你要轉換的報告類型，並上載相關的 PDF 檔案。本工具會自動將數據轉換為 Excel 格式，方便稍後貼上至 QSIP 的分析檔案中。
*Please select the report type and upload the corresponding PDF file. This tool will convert the data into Excel format, ready to be pasted into the QSIP analysis template.*
""")
...
# -----------------
with tab1:
    st.subheader("📝 項目分析報告轉換區 | Item Analysis Converter")
    ...
    with col1:
        st.info("""
        💡 **本區適用於以下格式的報告：**
        一般為數學科等科目，表格橫向列出「平均分 Mean」、「標準差 S.D.」等數據。
        
        **Applicable for reports formatted like:**
        *Typically for subjects like Mathematics. The table horizontally displays data such as 'Mean' and 'S.D.'.*
        """)
        if os.path.exists("example1_item.png"):
            st.image("example1_item.png", caption="報告格式參考 | Format Reference", use_column_width=True)
        else:
            ...
            with st.spinner("系統正在處理檔案，請稍候... | Processing file, please wait..."):
                try:
                    ...
                    if df_item.empty:
                        ...
                        st.success(f"✅ 提取成功！共獲取 {len(df_item)} 行數據。(系統已自動移除不必要的差距欄位) \n *Extraction successful! {len(df_item)} rows retrieved. (Unnecessary difference columns are automatically removed)*")
...
# -----------------
with tab2:
    st.subheader("✅ 多項選擇題報告轉換區 | MCQ Analysis Converter")
    ...
    with col3:
        st.info("""
        💡 **本區適用於以下格式的報告：**
        一般為英文科的 MCQ 報告，表格列出「A, B, C, D」選項的選擇人數，並附有 ☑️ 標記顯示正確答案。
        
        **Applicable for reports formatted like:**
        *Typically for English language MCQ reports. The table lists the number of students for options 'A, B, C, D' and uses a ☑️ mark to indicate the correct answer.*
        """)
        if os.path.exists("example2_mcq.png"):
            st.image("example2_mcq.png", caption="報告格式參考 | Format Reference", use_column_width=True)
        else:
            st.warning("⚠️ (提示: 系統未找到 example2_mcq.png | Image not found)")
            
    with col4:
        file_mcq = st.file_uploader("📂 請於此處上載「MCQ 分析」PDF | Upload 'MCQ Analysis' PDF here", type=["pdf"], key="file_mcq")
        ...
        if file_mcq is not None:
            with st.spinner("系統正在處理檔案，請稍候... | Processing file, please wait..."):
                try:
                    ...
                    if df_mcq.empty:
                        st.error("❌ 無法提取數據！請確認你上載的是否為正確的「MCQ 分析報告」。 \n *Failed to extract data! Please ensure you uploaded the correct 'MCQ Analysis Report'.*")
                    else:
                        st.success(f"✅ 提取成功！共獲取 {len(df_mcq)} 題的數據。(系統已自動辨識正確答案) \n *Extraction successful! Data for {len(df_mcq)} questions retrieved. (Correct answers automatically identified)*")
...
st.caption("""
📌 **小貼士 Tips:** 
下載 Excel 後，請打開檔案並全選數據，然後直接貼上 (Ctrl+C 及 Ctrl+V) 至 QSIP 提供的分析範本內。
*After downloading the Excel file, open it, select all data, and directly paste (Ctrl+C & Ctrl+V) it into the provided QSIP analysis template.*
""")
